...
function formatSecondsToHMS(seconds: number) {
  const sec = Math.floor(seconds); // force integer seconds
...
// Returns the current week key in the format "YYYY-W{weekNumber}"
...
export default function DashboardPage() {
  ...
  // State for weekly progress chart (time watched in seconds)
  ...
  // State for the count of unique videos watched during the week
  const [uniqueVideosWatched, setUniqueVideosWatched] = useState(0);

  // Read weekly progress from localStorage on mount
  useEffect(() => {
    ...
    if (storedData) {
      ...
      // Build chart data using raw seconds (timeSpent)
      ...
      // Calculate unique video count across the week
  ...
  if (isUserLoading || isLecturesLoading || !dashboard || !user) {
    return (
  ...
  return (
    <div className="container mx-auto py-10">
      ...
      {/* Stats grid including a card for unique videos watched */}
